...
from trainer.preKTNetTrainer import preKTNetTrainer
from trainer.disctrainier import DiscriminatorTrainer
from trainer.Zero_shot import ZeroShot
...
flags.DEFINE_string('phase','meta','pre, meta, disc or zero')


flags.DEFINE_string('exp_string', ' ', 'experiment string')
flags.DEFINE_string('pre_string', ' ', 'pretrain string')
flags.DEFINE_string('pretrain_dir', ' ', 'pretrain directory')
...
flags.DEFINE_integer('pretrain_class_num', 60, 'number of classes used in the pre-train phase')
flags.DEFINE_integer('pretrain_batch_size', 16, 'batch_size for the pre-train phase')
flags.DEFINE_integer('pretrain_iterations', 40000, 'number of pretraining iterations.')
flags.DEFINE_integer('pre_sum_step', 10, 'the step number to summary during pretraining')
flags.DEFINE_integer('pre_save_step', 500, 'the step number to save the pretrain model')
flags.DEFINE_integer('pre_print_step', 100, 'the step number to print the pretrain results')
...
flags.DEFINE_string('pretrain_folders', './data/FC100/train', 'directory for pre-train data')
...
flags.DEFINE_integer('shot_num', 15, 'number of examples per class (K for K-shot learning)')
flags.DEFINE_integer('metatrain_epite_sample_num', 7, 'number of meta train episode-test samples')
flags.DEFINE_integer('metatest_epite_sample_num', 15, 'number of meta test episode-test samples, 0 means metatest_epite_sample_num=shot_num')
flags.DEFINE_integer('meta_sum_step', 10, 'the step number to summary during meta-training')
flags.DEFINE_integer('meta_save_step', 100, 'the step number to save the model')
flags.DEFINE_integer('meta_intrain_val_sample', 20, 'the number of samples used for val during meta-train')
...
flags.DEFINE_integer('metatrain_iterations', 20000, 'number of meta-train iterations.')
...
flags.DEFINE_integer('test_iter', 100, 'iteration to load model')
...
flags.DEFINE_string('metatrain_dir', './data/FC100/train', 'directory for meta-train set')
flags.DEFINE_string('metaval_dir', './data/FC100/val', 'directory for meta-val set')
flags.DEFINE_string('metatest_dir', './data/mini-imagenet/test', 'directory for meta-test set')

flags.DEFINE_string('meta_zerotest_dir', './data/mini-imagenet/zerotest', 'directory for meta-zerotest set')
flags.DEFINE_string('meta_dtrain_dir', './data/mini-imagenet/dtrain', 'directory for meta-zerotest set')
flags.DEFINE_string('meta_dvalid_dir', './data/mini-imagenet/dvalid', 'directory for meta-zerotest set')
flags.DEFINE_string('meta_dtest_dir', './data/mini-imagenet/dtest', 'directory for meta-zerotest set')

flags.DEFINE_string('activation', 'leaky_relu', 'leaky_relu, relu, or None')
flags.DEFINE_string('norm', 'batch_norm', 'batch_norm, layer_norm, or None')
flags.DEFINE_bool('metatrain', False, 'is this the meta-train phase')
...
def main():
    ...
    if FLAGS.phase=='pre':
        FLAGS.pre_string = 'logs'
        trainer = preKTNetTrainer()
    elif FLAGS.phase=='meta':
        trainer = MetaTrainer()    
    elif FLAGS.phase=='disc':
        trainer = DiscriminatorTrainer()  
    elif FLAGS.phase=='zero':
        trainer = ZeroShot()      
