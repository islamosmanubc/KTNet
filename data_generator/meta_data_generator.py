...
from utils.misc import get_images, process_batch, process_batch_augmentation,data_augment
...
class MetaDataGenerator(object):
    ...
    def generate_data(self, data_type='train'):
        ...
        if data_type=='train':
            ...
            if FLAGS.metatest_epite_sample_num==0:
                ...
                num_samples_per_class = FLAGS.shot_num + FLAGS.metatest_epite_sample_num
        elif data_type=='zerotest':          
            metatest_folder = FLAGS.meta_zerotest_dir
            folders = [os.path.join(metatest_folder, label) \
                for label in os.listdir(metatest_folder) \
                if os.path.isdir(os.path.join(metatest_folder, label)) \
                ]
        ...
        if not os.path.exists(self.this_setting_filename_dir+'/' + data_type + '_fcdata.npy'):
            ...
            np.save(self.this_setting_filename_dir+'/' + data_type + '_fcdata.npy', data_list)
    ...
    def load_data(self, data_type='test'):
        ...
        Arg:
          data_type: the phase for meta-learning.
        """
        data_list = np.load(self.this_setting_filename_dir+'/' + data_type + '_fcdata.npy', allow_pickle=True, encoding="latin1")
        if data_type=='train':
    ...
    def processepisode(self,this_inputa,this_inputb,this_labela,this_labelb,dtype):
        inputa = []
        labela = []

        
        labelan = np.argmax(this_labela,1)
        sortedindices = np.argsort(labelan)
        this_inputa = this_inputa[sortedindices]
        this_labela = this_labela[sortedindices]

        for j in range(len(this_inputa)):
            for i in range(len(this_labelb)):
                labela.append(this_labela[j])

        labels = []

        for j in range(len(this_inputa)):
            for i in range(len(this_labelb)):
                if(np.argmax(labela[len(this_labelb)*j+i]) == np.argmax(this_labelb[i])):
                    labels.append(1)
                else:
                    labels.append(0)
        
        inputa = np.array(this_inputa)
        labela = np.array(this_labela)
        labels = np.array(labels)



        return inputa,labela, labels
    ...
    def load_episode(self, index, data_type='train'):
        ...
        if data_type=='train':
            ...
            if FLAGS.metatest_epite_sample_num==0:
                ...
                epite_sample_num = FLAGS.metatest_epite_sample_num
        elif data_type=='val':
        ...
        if FLAGS.metatrain is False and FLAGS.base_augmentation:
            ...
            this_inputb, this_labelb,this_input_aug = data_augment(this_task_te_filenames, \
                this_task_te_labels, dim_input, epite_sample_num)
            templabel = this_labelb
            inputb_disc,labelb, labelssl = self.processepisode(this_inputb,this_input_aug,this_labelb,this_labelb,data_type)
            inputa_disc,labela, labeldisc = self.processepisode(this_inputa,inputb_disc,this_labela,labelb,data_type)
            
        else:

            this_inputa, this_labela,this_input_aug = data_augment(this_task_tr_filenames, \
                this_task_tr_labels, dim_input, epitr_sample_num)
            templabel = this_labela
            inputa_disc,labela, labelssl = self.processepisode(this_inputa,this_input_aug,this_labela,this_labela,data_type)
            
            this_inputb, this_labelb = process_batch(this_task_te_filenames, \
                this_task_te_labels, dim_input, epite_sample_num)
            
            inputa_disc,labela, labeldisc = self.processepisode(inputa_disc,this_inputb,labela,this_labelb,data_type)
            
        return inputa_disc, labela, this_inputb, this_labelb, labeldisc, this_input_aug, labelssl,templabel
