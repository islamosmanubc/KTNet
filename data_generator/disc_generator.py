...
class DiscDataGenerator(object):
    ...
    def processepisode(self,this_inputa,this_inputb,this_labela,this_labelb,dtype):
        ...
        for j in range(FLAGS.way_num):
            inputa.append(this_inputa[j])
            labela.append(this_labela[j])
            stop = False
            for i in range(len(this_inputb)):
                if(stop == True and np.argmax(this_labelb[i]) == j):
                    inputb.append(this_inputb[i])
                    labelb.append(this_labelb[i])
                if(stop == False and np.argmax(this_labelb[i]) == j):
                    inputa.append(this_inputb[i])
                    labela.append(this_labelb[i])
                    stop = True
        ...
        for i in range(len(inputa)):
            if(np.argmax(labela[i]) == np.argmax(labelb[i])):
                labels.append(1)
            else:
                labels.append(0)
    ...
    def load_episode(self, index, data_type='train'):
        ...
        if FLAGS.metatrain is False and FLAGS.base_augmentation:
            ...
            this_inputa, this_inputb, labeldisc = self.processepisode(this_inputa,this_inputb,this_labela,this_labelb,data_type)

        return this_inputa, this_inputb, labeldisc
