...
def data_augment(input_filename_list,input_label_list,dim_input,batch_sample_num):
    ...
    for filepath in new_path_list:
        ...
        this_img_h = []
        img = this_img.reshape(1,84,84,3)
        r = randrange(4) 
        if(r == 0):
            this_img_h = central_scale_images(img, [0.90, 0.75, 0.60],84,CH)
        if(r == 1):
            this_img_h = flip_images(img,84,CH)
        if(r == 2):
            this_img_h = translate_images(img,84,CH)
        if(r == 3):
            this_img_h = rotate_images(img,84,CH)
    ...
    img_array_h = np.array(img_list_h).reshape([FLAGS.way_num*batch_sample_num, dim_input])

    return img_array, label_array, img_array_h
...
def resnet_conv_block_wb(inp, cweight, reuse, scope, activation=leaky_relu):
    """The function to forward a conv layer.
    Args:
      inp: the input feature maps.
      cweight: the filters' weights for this conv layer.
...
def contrastive_loss(model1, model2, y, margin):
		return tf.reduce_mean(input_tensor=dissimilarity + similarity) / 2,similarity,dissimilarity
...
def softmaxloss(pred, label):
    ...
    Return:
      The Loss.
    """
    return tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=tf.stop_gradient(label)))

def mixedloss(pred, label,disclabel):
    
    softmax_loss = softmaxloss(pred,label)
...
def xent(pred, label):
    ...
    """
    return tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=tf.stop_gradient(label)) / FLAGS.shot_num
...
def SQ(pred, label):
    ...
    Note: with tf version <=0.12, this loss has incorrect 2nd derivatives
    """
    return tf.nn.l2_loss(pred-label) / FLAGS.shot_num
