        ...
        def construct_pretrain_model(self, input_tensors=None, is_val=False):
            ...
            with tf.variable_scope('pretrain-model', reuse=None) as training_scope:
                ...
                if is_val is False:
                    ...
                    self.pretrain_op = optimizer.minimize(self.pretrain_task_loss, var_list=list(weights.values())+list(fc_weights.values()))
                    ...
                    x = tf.argmax(self.label, 1)
                    y = tf.nn.softmax(self.pretrain_task_output_val)
                    z=tf.argmax(y, 1)
                    self.pretrain_task_accuracy_val = tf.reduce_mean(tf.contrib.metrics.accuracy(z, x))
